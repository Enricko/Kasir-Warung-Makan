import 'package:kasir_warung_makan/app/data/product_model/product_model.dart';
import 'package:kasir_warung_makan/app/modules/payment_receipt/views/payment_receipt_view.dart';
...
class HomeController extends GetxController {
  ...
  final pecel = const ProductModel(harga: 12000, nama: 'Pecel').obs;
  ...
  void resetAll() {
    pecel.value = const ProductModel(harga: 12000, nama: 'Pecel');
  ...
  void getTotalBayar() {
    totalBayar.value = (pecel.value.harga * pecel.value.count) +
  ...
  void addpecel({int count = 0}) {
    ProductModel productModel =
        const ProductModel(harga: 12000, nama: 'pecel');
    if (count == 0) {
      int count = pecel.value.count + 1;
      pecel.value = productModel.copyWith(count: count);
    } else {
      pecel.value = productModel.copyWith(count: count);
    }
    getTotalBayar();
  ...
  void removepecel({int count = 0}) {
    ProductModel productModel =
        const ProductModel(harga: 12000, nama: 'pecel');
    if (pecel.value.count - 1 < 0) {
      return;
    }
    if (count == 0) {
      int count = pecel.value.count - 1;
